...
def precompute_spectrograms(filename_sw, EEG_channels = None, acqs = None):
	...
	Requires the downsampled per-hour EEG files to already exist (run
	downsample_filter first).
	"""
	if isinstance(filename_sw, dict):
		d = filename_sw
	else:
...
if __name__ == "__main__":
	...
	if len(args) < 2:
		...
		spect_flag = input('Do you want to pre-compute all spectrograms now (fast GUI startup)? (y/n)') == 'y'
		if spect_flag:
			precompute_spectrograms(args[1])
